...
BOOL CProductStep1Dlg::OnInitDialog()
{
	...
	m_TechValList.InsertColumn(0,CString("序号"),LVCFMT_CENTER,width1/12);
	m_TechValList.InsertColumn(1,CString("采用工艺"),LVCFMT_CENTER,width1/6);
	m_TechValList.InsertColumn(2,CString("要求分类"),LVCFMT_CENTER,width1/6);
	m_TechValList.InsertColumn(3,CString("工艺指标"),LVCFMT_CENTER,width1/6);
	m_TechValList.InsertColumn(4,CString("评分"),LVCFMT_CENTER,width1/3);
	m_TechValList.InsertColumn(5,CString("权重"),LVCFMT_CENTER,width1/12);
	...
	nComboLis.push_back(4);            //第四列有组合框控制
...
{
	_RecordsetPtr m_pRs,m_pRs0,m_pRs1;
	m_Lvl3TechID.clear();
	m_Lvl4TechID.clear();
	int UpID=5;             //搜寻工艺指标（指标ID为5）的子项
	CString strUpID=(CString)(char *)(_bstr_t)(UpID);

	//CString strProcedureID;
	//CString sql = CString("select * from Procedure where EvalModelID=")+m_ProductInfo.str_EvalType;
	//try
	//{
 //   m_pRs=theApp.m_pConnect->Execute(_bstr_t(sql), NULL, adCmdText);
	//strProcedureID = (CString)(m_pRs->GetCollect("ProcedureID"));
	//}
	//catch(_com_error e)
	//{
	//	CString temp;
	...
	CString sql0 = CString("select * from CreateIndexTree where ProcedureID=")+m_ProductInfo.str_EvalType+CString("and TechEvalIndexUpID=")+strUpID;
	try{
	...
	{
		CString sql1 = CString("select * from CreateIndexTree where ProcedureID=")+m_ProductInfo.str_EvalType+CString("and TechEvalIndexUpID=")+m_Lvl3TechID[i];
...
void CProductStep1Dlg::ShowListCtrl(vector<vector<CString>>& m_Lvl4TechID)
{
	_RecordsetPtr m_pRs,m_pRs1,m_pRs2;
	m_ListCtrlItem.clear();
	vector<CString> m_Lvl3TechNam;    //存储三级指标名
	//vector<vector<CString>> m_Lvl4TechNam;
	for (int i=0;i<m_Lvl4TechID[0].size();i++)
	{
		CString sql = CString("select * from TechEvalIndex where TechEvalIndexID=")+m_Lvl4TechID[0][i];
		try
		{
		m_pRs = theApp.m_pConnect->Execute(_bstr_t(sql), NULL, adCmdText);
		CString strTechNam=(CString)(m_pRs->GetCollect("TechEvalIndexNam"));
		m_Lvl3TechNam.push_back(strTechNam);
		}
		catch(_com_error e)
		{
			CString temp;
...
void CProductStep1Dlg::SetListItem(vector<CTechChartItem>& m_ListCtrlItem)
{
	...
	for (int i=0;i<m_ListCtrlItem.size();i++)
	{
		...
		m_TechValList.SetItemText(i,1,m_ListCtrlItem[i].m_ChooseTech);
		m_TechValList.SetItemText(i,2,m_ListCtrlItem[i].m_ReqClassify);
		m_TechValList.SetItemText(i,3,m_ListCtrlItem[i].m_TechEvalIndex);
		m_TechValList.SetComboString(m_ListCtrlItem[i].m_ValComboStr);
